...
M.mark_differences = function(file_to_golf)
  -- Go to GOLF_FILE_META.txt and find this info instead of passing to func
  local first_file = vim.fn.readfile(file_to_golf)
  local second_file = vim.fn.readfile 'GOLF_FILE.txt'
  ...
  for line_number = 1, #first_file do
    ...
    if line_difference > 0 then
      for second_line_number = #second_file - line_difference, #second_file do
        vim.fn.sign_place(0, 'golf_group', 'GolfMarker', vim.api.nvim_get_current_buf(), { lnum = second_line_number })
        mismatch_counter = mismatch_counter + 1
      end
    end
  end
