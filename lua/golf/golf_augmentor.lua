local M = {}
-- This function reads in an incoming file as a table of strings
-- It then applies some random text alteration
-- The goal is to mostly keep the shape of the source, but
-- It should also allow the user to practice some NeoVim motions
-- There is quite a bit of scaffolding in the function that is not used
-- My idea was to rank some entropy functions numerically
-- And have each function reduce a budget giving by the entropy_level
-- Other priorities came up so I just chose a coin flip between 2 functions
-- This can definitely be made better
M.augment_file = function(incoming_file, golf_file_location, entropy_level)
	...
	vim.fn.writefile(augmented_golf_file, golf_file_location)
end
